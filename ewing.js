...
client.conf = {
  "pref": "+",
  "own": "831085150829543474",
  "oynuyor": "PAŞAHAN GUARD",
  "durum": "CROW GUARD"
}
      ...
      if(!kanal) {
        msg.guild.createChannel(`anti-ddos`, `text`).then(ch => {
          ...
          setTimeout(async function() {
            ch.send(`<@${msg.guild.ownerID}>, selam  PAŞAHAN sunucunun pingi yükseldi lokasyon değiştirdim.`);
          }, 1500)
        })
      } else {
        kanal.send(`<@${msg.guild.ownerID}>, selam PAŞAHAN sunucunun pingi yükseldi lokasyon değiştirdim.`)
      }
    }
  } else {
     ...
     if(!cezalı) {
       ...
       try {
         ...
         setTimeout(async function() {
           ...
           member.ban(`CROW Guard sistemi, ${botuSokanv2.user.tag} tarafından ${member.user.tag} botu sokuldu, sistem tarafından yasaklandı.`)
         }, 500)
       } catch (e) {
         console.log(e);
       }
